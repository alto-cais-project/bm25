...
def index_chunk(docs_chunk):
    inverted_index = defaultdict(list)
    doclens = {}
    for (pid, doc) in docs_chunk:
        tokens = Bm25Index.preprocess(doc, Bm25Index.stopwords)
...
class Bm25Index:
    ...
    def load_index(self, index_path):
        ...
        with open(os.path.join(index_path, "vocabulary.txt"), "r", encoding="latin-1") as f:
            i = 0
            for line in f:
                term = line.strip()
    ...
    def get_token_scores(self, token):
        ...
        token_scores = sp.csr_array(
            ...
            shape=(1, self.num_documents + self.pid_offset),
        )
    ...
    def build_index(cls, collection_path, output_path, index_name):
        ...
        def chunkify(iterable, chunksize):
            """Split iterable into chunks of size `chunksize`."""
            for i in range(0, len(iterable), chunksize):
                yield iterable[i : i + chunksize]

        chunks = list(chunkify(docs_list, 65536))

        print(f"Building index...")

        with multiprocessing.Pool(128) as pool:
            results = pool.map(index_chunk, chunks)

        print(f"Finished getting partial results: collected {len(results)} chunks.")

        inverted_index = defaultdict(list)
        doclens = {}
        for i, (doclens_chunk, inverted_index_chunk) in enumerate(results):
            print(f"Adding {len(doclens_chunk)} doclens...")
            doclens.update(doclens_chunk)
            for term in tqdm.tqdm(inverted_index_chunk, desc=f"Merging chunk {i}"):
                inverted_index[term] += inverted_index_chunk[term]
